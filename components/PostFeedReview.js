    ...
    return (
        ...
        <div className='mb-4 rounded-lg shadow-lg bg-white overflow-clip lg:flex'>
            ...
            <figure className='flex-none w-full h-60 lg:w-60 lg:max-h-52 shadow-xl relative'>
                <Image src={post?.imageThumbnail ?? '/luffi.jpg'} fill alt={post?.title ?? 'title'} sizes="(max-width: 240px) 33vw" priority={true}/>
            ...
            <div className='p-4'>
                ...
                <p className="text-xl leading-relaxed py-1">
                    {post?.description ?? getSnippet(post?.content, post?.titlen)}
                </p>

                <footer>
                    <span>
                        <Link href={`/review/${post?.uid}/${post?.slug}`}>
                            <strong className="font-bold font-sans hover:text-slate-400 px-4 py-2 border rounded-md bg-slate-50">
                                REVIEW
                            </strong>
                        </Link>
                    </span>
